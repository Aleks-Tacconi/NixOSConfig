...
CURRENT_WALLPAPER_LINK="$HOME/.wallpaper-current"
NO_GNOME_FLAG=""
...
set_current_wallpaper_link() {
    local path="$1"

    if [ -n "$path" ] && [ -e "$path" ]; then
        ln -sfn "$path" "$CURRENT_WALLPAPER_LINK"
    fi
}
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            if [ -f "$thumbnail" ]; then
                ...
                set_current_wallpaper_link "$thumbnail"
            ...
            set_wallpaper_path "$imgpath"
            set_current_wallpaper_link "$imgpath"
            remove_restore
        fi
    fi
