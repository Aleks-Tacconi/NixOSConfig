...
ListView {
    ...
    property bool customWheelScrolling: Config?.options.interactions.scrolling.fasterTouchpadScroll ?? false
    ...
    WheelHandler {
        id: wheelHandler
        enabled: root.customWheelScrolling
        target: root
        orientation: Qt.Vertical
        acceptedDevices: PointerDevice.Mouse | PointerDevice.TouchPad
        onWheel: event => {
            const delta = event.angleDelta.y / root.mouseScrollDeltaThreshold;
            const isMouse = Math.abs(event.angleDelta.y) >= root.mouseScrollDeltaThreshold;
            const scrollFactor = isMouse ? root.mouseScrollFactor : root.touchpadScrollFactor;
