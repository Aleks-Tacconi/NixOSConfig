...
ColumnLayout {
    ...
    property var syntaxDefinition: Repository.definitionForName(root.displayLang || "plaintext")
    property string displayLangName: root.syntaxDefinition?.name ?? (root.displayLang || "plain")
    property string displayContent: root.editing ? String(root.segmentContent ?? "") : String(root.segmentContent ?? "").replace(/\n+$/, "")
    ...
    RowLayout { // Line numbers and code
        ...
        Rectangle { // Line numbers
            ...
            ColumnLayout {
                ...
                Repeater {
                    model: codeTextArea.text.length > 0 ? codeTextArea.text.split("\n").length : 1
                    Text {
        ...
        Rectangle { // Code background
            ...
            ColumnLayout {
                ...
                Flickable {
                    ...
                    TextArea { // Code
                        id: codeTextArea
                        width: Math.max(codeFlickable.width, contentWidth + leftPadding + rightPadding)
                        readOnly: !editing
                        ...
                        SyntaxHighlighter {
                            ...
                            definition: root.syntaxDefinition ?? Repository.definitionForName("plaintext")
                    ...
                    MouseArea {
                        ...
                        onWheel: wheel => {
                            ...
                            if (Math.abs(horizontalDelta) > Math.abs(verticalDelta) && horizontalDelta !== 0) {
                                const maxContentX = Math.max(0, codeFlickable.contentWidth - codeFlickable.width);
                                codeFlickable.contentX = Math.max(0, Math.min(maxContentX, codeFlickable.contentX - horizontalDelta));
                                wheel.accepted = true;
                                return;
                            }
