...
ColumnLayout {
    ...
    onSegmentContentChanged: {
        ...
        shownText = renderedSegmentContent;
    ...
    Repeater {
        ...
        model: ScriptModel {
            ...
            onValuesChanged: {
                if (textLinesRepeater.textLineOpacities.length > values.length) {
                    textLinesRepeater.textLineOpacities = textLinesRepeater.textLineOpacities.slice(0, values.length);
                }
        ...
        delegate: TextArea {
            ...
            text: !root.editing ? modelData.replace(/\s+$/, "") : modelData
